...
Input: [
        ...
        start_nn: str (default '0'). Skip all values that are less than this attribute.
    ]
Output: [
...
max_length = int(argv[3]) if len(argv) > 3 else 3
min_length = int(argv[4]) if len(argv) > 4 else 0
first_value = str(argv[5]) if len(argv) > 5 else '0'

driver = webdriver.Firefox()
driver.get("https://github.com/settings/admin")
login_input = driver.find_element_by_id("login_field")
password_input = driver.find_element_by_id("password")
login_input.send_keys(argv[1])
password_input.send_keys(argv[2])
